...
def scale_card(reader: SensorReader, cal_weight: int):
    with ui.card():
    ...
    with ui.card().style("min-width: 20em"):
        ...
        ui.label("Countdown:")
        ui.circular_progress(0, min=0, max=reader.scale.stable_measurements, color="red").bind_value_from(reader.scale,
                                                                                                   'measure_countdown').props(
            "instant-feedback")

        ui.separator()

        ui.label("...").bind_text_from(reader.scale, 'last_stable_weight', backward=lambda v: f"Measured: {v:.2f}g")
...
async def calibrate_cat_page():
    ...
    scale_card(meowton.cat_reader, 200)
...
async def calibrate_food_page():
    ...
    scale_card(meowton.food_reader, 10)
